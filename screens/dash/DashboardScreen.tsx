...
const registerForPushNotificationsAsync = async (): Promise<void> => {
  if (Constants.isDevice) {
    ...
    Notifications.getExpoPushTokenAsync().then((resp) => {
      fetch("http://192.168.1.41:9090" + "/me", {
        method: "POST",
        headers: {
        ...
        .then((response) => {
          response.json().then((responseJson) => {
            ...
            fetch("http://192.168.1.41:9090" + "/putDeviceId", {
              method: "POST",
              headers: {
                Authorization: global.token,
                "content-type": "application/json",
    ...
    <Layout style={[styles.container, { paddingTop: safeArea.top }]} level="2">
      ...
      <View style={{ flex: 1 }}>
        <List
          ...
          data={data}
          ListHeaderComponent={
            <TopNavigation alignment="center" title="Mission En Cour" />
          }
          stickyHeaderIndices={[0]}
          renderItem={renderItem}
        />
        <TopNavigation alignment="center" title="Mission Terminée" />
        <List
          style={styles.list}
          contentContainerStyle={styles.listContent}
          data={data}

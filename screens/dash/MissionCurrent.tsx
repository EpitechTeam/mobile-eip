...
export default ({ navigation }): React.ReactElement => {
  ...
  const registerForPushNotificationsAsync = async (): Promise<void> => {
    try {
      if (Constants.isDevice) {
        const status0 = await Permissions.getAsync(Permissions.NOTIFICATIONS);
        let finalStatus = status0;
        if (status0.status != "granted") {
        ...
        console.log("willally toekn => " + global.token);
        ...
        Notifications.getExpoPushTokenAsync()
          .then((resp) => {
            fetch(global.BaseUrl + "/me", {
              ...
              .catch((error) => {
                console.error("error in fetching /me" + error);
              });
            console.log("expo push token => " + resp);
          })
          .catch((error) => {
  ...
  const renderItemHeader = (info): React.ReactElement => (
    ...
    >
      <ImageOverlay style={styles.itemHeader} source={{ uri: info.item.img }}>
        <View style={styles.itemHeaderDetails}>
          <Text category="h4" status="control">
            {`${info.item.house.name}`}
          </Text>
        </View>
        <Text style={{ paddingTop: 50 }} category="s1" status="control">
  ...
  const renderItemFooter = (info): React.ReactElement => (
    <View style={styles.itemFooter}>
      <View style={styles.itemReactionsContainer}>
        <Text style={{}}>Rémunération</Text>
      </View>
      <Text style={{}}>{`${info.item.deal} €`}</Text>
    </View>
  );
  ...
  const renderDetailItem = (detail: string): React.ReactElement => (
    <Button style={styles.detailItem} appearance="outline" size="tiny">
      {detail}
    </Button>
  );
  ...
  const renderItem = (info): React.ReactElement => (
    ...
    >
      ...
      <View style={styles.detailsList}>{renderDetailItem(info.item.name)}</View>
    </Card>
  );

  return (
    <Layout style={[styles.container, { paddingTop: safeArea.top }]} level="2">
      <NavigationEvents onWillFocus={(payload) => onEndInput()} />
      <List
        style={styles.list}
